...
int main() {
    pthread_t http_thread, websocket_thread, tcp_thread;
    MYSQL *db_conn;
    ...
    // TCP 서버 쓰레드 생성
    if (pthread_create(&tcp_thread, NULL, (void *(*)(void *))start_tcp_server, NULL) != 0) {
        perror("Error creating TCP server thread");
        return 1;
    }
